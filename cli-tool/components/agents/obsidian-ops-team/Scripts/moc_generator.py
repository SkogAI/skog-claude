...
"""
        ...
        if stats["common_topics"]:
            ...
            sorted_topics = sorted(
                stats["common_topics"].items(), key=lambda x: x[1], reverse=True
            )
...
"""
    ...
    def organize_files_by_topic(self, directory_path, stats):
        ...
        sorted_topics = sorted(
            topic_files.items(), key=lambda x: len(x[1]), reverse=True
        )
...
def main():
    ...
    parser.add_argument("--directory", help="Specific directory to create MOC for")
    parser.add_argument("--title", help="Title for the MOC")
    parser.add_argument("--description", help="Description for the MOC")
    parser.add_argument(
        "--suggest", action="store_true", help="Suggest directories that need MOCs"
    )
    parser.add_argument(
        "--create-all",
        action="store_true",
    ...
    if args.suggest or args.create_all:
        ...
        if args.suggest:
            ...
            for suggestion in suggestions:
                ...
                if suggestion["top_topics"]:
                    topics = [
                        f"{topic} ({count})"
                        for topic, count in suggestion["top_topics"]
                    ]
        ...
        if args.create_all:
            for suggestion in suggestions:
                generator.create_moc_file(suggestion["directory"], suggestion["title"])
