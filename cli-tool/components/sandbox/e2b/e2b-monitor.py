...
def monitor_sandbox_execution(sbx, command, timeout=600):
    ...
    try:
        ...
        if result.stderr:
            log_with_timestamp(
                f"STDERR length: {len(result.stderr)} characters", "WARNING"
            )
...
def monitor_file_system(sbx, description="Monitoring file system"):
    ...
    try:
        ...
        find_result = sbx.commands.run(
            "find . -type f -name '*.html' -o -name '*.js' -o -name '*.css' -o -name '*.json' | head -10",
            timeout=15,
        )
...
def enhanced_sandbox_execution(
    prompt, components_to_install="", e2b_api_key=None, anthropic_api_key=None
):
    """
    ...
    try:
        ...
        if claude_check.exit_code == 0:
            version_check = monitor_sandbox_execution(
                sbx, "claude --version", timeout=10
            )
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "Usage: python e2b-monitor.py <prompt> [components] [e2b_key] [anthropic_key]"
        )
        print()
        print(
            "This tool provides enhanced monitoring and debugging for E2B sandbox operations."
        )
        print(
            "Use this when you have valid API keys and want detailed insight into sandbox execution."
        )
        sys.exit(1)
